...
// 向左移动距离（米，相对机器人坐标系，左方向为正y）
const float LEFT_MOVE_Y = 0.10;

// 创建服务客户端
ros::ServiceClient relmove_client;

// 相对位移（向左）：y 为相对机器人坐标系的左右距离，正值向左
...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "left_move_node");
    ros::NodeHandle nh;
    ...
    // 执行向左移动 0.20m
    ROS_INFO("开始向左移动 %.2f m...", LEFT_MOVE_Y);
    if (!set_relmove(0, LEFT_MOVE_Y, 0)) {
        ROS_ERROR("向左移动失败！");
        return -1;
    }
